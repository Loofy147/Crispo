...
class CodeGenerator:
    ...
    def generate(self, objective: str, trust_parameter: float = 0.8) -> str:
        ...
        if any(kw in objective for kw in ['fetch', 'get', 'api', 'request']):
            return self._generate_api_template(0)
        if any(kw in objective for kw in ['process', 'transform', 'clean', 'pandas']):
            return self._generate_transform_template(1)
        if any(kw in objective for kw in ['analyze', 'numpy', 'compute', 'calculate']):
            return self._generate_high_complexity_template(2)

        # Fallback for generic objectives
        return self._generate_simple_template(0)
...
'''
    ...
    def _generate_high_complexity_template(self, layer_id: int) -> str:
...
if __name__ == '__main__':
    ...
    if 'pipeline_context' not in locals():
        pipeline_context = {{'data': np.random.rand(1, 2).tolist()}}
    result = system.process(pipeline_context)
    print(json.dumps(result))
'''

    def _generate_transform_template(self, layer_id: int) -> str:
